<template>
   <div class="h-100 d-flex flex-column">
      <led-canvas id="canvas" class="bg-secondary" :frame="frame" @drawError="onDrawError($event)" />
   ...
   async setup() {
      ...
      const animation: Partial<Animation> =
         route.params['animationId'] !== 'new'
            ? deepClone(
               await animationClient.latest(
                  route.params['animationId'] as string,
                  true
               )
            )
            : {
      ...
      const errorMessages = computed(() => {
         if (scriptParseResult.value.valid === false) {
            return scriptParseResult.value.errors.filter(
               (e) => e !== 'Script parsing failed'
            );
         }
         return [];
      });
      ...
      const testScript = async () => {
         ...
         try {
            ...
            intervalTimeout = setInterval(async () => {
               try {
                  frame.value = [
                     ...(await iframeContext.value!.nextFrame()),
                  ];
               } catch (e) {
      ...
      const contentStopHandle = watch(
         content,
         (s) => (animationPost.script = s)
      );
