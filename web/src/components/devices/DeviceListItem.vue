   ...
   <div class="card h-100">
      <div class="card-header">
         <div class="row">
            ...
            <div class="col-auto">
               <button class="btn p-0 ms-2 text-success" v-if="deviceCopy.isStopped" @click.prevent="stop(deviceCopy, false)">
                  <i class="fas fa-play fa-fw"></i>
               </button>

               <button class="btn p-0 ms-2 text-danger" v-if="!deviceCopy.isStopped" @click.prevent="stop(deviceCopy, true)">
      ...
      <div class="card-body">
         <div class="form-floating">
            <select id="device-animation" class="form-select" placeholder="*" v-model="selectedConfigId">
               <option value>None</option>
               <option v-for="c of configs" :key="c.id" :value="c.id">{{ c.animationName }} - {{ c.name }}</option>
            </select>
            <label for="device-animation">Animation</label>
         </div>

         <router-link class="small" v-if="selectedConfigId" :to="{ name: 'animation-config', params: { configId: selectedConfigId } }">Edit Config</router-link>
...
<script lang="ts">
   ...
   import { AnimationNamedConfigSummary, deepClone, Device, DeviceHealthData, Disposable, Id } from '@krakerxyz/netled-core';
      ...
      setup(props) {
         ...
         disposables.push(
            ws.on('deviceMessage', (msg) => {
               if (msg.deviceId !== props.device.id) {
                  return;
               }
               if (msg.type === 'health') {
